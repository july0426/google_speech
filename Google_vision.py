    ...
    if webEntities:
        ...
        for web in webEntities:
            if 'description' in web.keys():
                web_dict = {web['description']: web['score']}
                web_list.append(web_dict)
    web_json = {"webEntities": web_list}
    print web_json
    return web_json
...
def goole_vision():
    ...
    textAnnotations = jsonpath.jsonpath(res_json,'$..textAnnotations')
    if textAnnotations:
        textAnnotations = textAnnotations[0]
        print textAnnotations,type(textAnnotations)
        langu = textAnnotations[0]['locale']
        sql = 'update pngtree_pdts_test set labels="%s",webEntities="%s",langu="%s" where id="%s"' % (labels,webEntities,langu,id)
    else:
